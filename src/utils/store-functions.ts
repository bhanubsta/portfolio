...
export const handleText = ({
  ...
  if (userInput === "Enter") {
    ...
    // Handle Terminal Editor Trigger Command
    const isEditorCmd = text.includes("-msg") || text === "mail" || text === "compose";
    if (isEditorCmd) {
      const { themeStore } = require("./store");
      const { setIsEditorOpen, setEditorMetadata, editorMetadata } = themeStore.getState();
      
      // Extract optional email pre-fill
      const parts = text.split(/\s+/);
      const email = parts.length > 2 ? parts[2] : "";
      if (email) setEditorMetadata({ ...editorMetadata, to: email });
